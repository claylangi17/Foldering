...
def fetch_distinct_layers_from_db(
    layer_level_to_fetch: int,  # 1 for L1, 2 for L2, 3 for L3
    ...
    For L2/L3, parent_layer_definition_pk is the 'id' of the parent layer_definition.
...
def fetch_items_for_layer_from_db(layer_definition_pk: int) -> List[Dict[str, Any]]:
    """
    ...
    # 1. Get layer_name_db and cluster_label_id from layer_definitions table
    get_layer_info_query = f"SELECT layer_name_db, cluster_label_id FROM {DEFINITIONS_TABLE_NAME} WHERE id = %s"
    try:
    ...
    query_items = f"""
        SELECT 
            ...
            po.PR_Ref_A,
            po.PR_Ref_B,
            po.Term_Payment_at_PO, -- Column name in DB might be Term_Payment_at_PO or Term_Payment
            po.RECEIVED_DATE,
            po.Sum_of_Order_Amount_IDR,
            po.Total_Cumulative_QTY_Order,
            po.Total_Cumulative_IDR_Amount,
            po.Checklist,
            po.Keterangan
        ...
        LIMIT 100 -- Consider pagination in the future
    """
